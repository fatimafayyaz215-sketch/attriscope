    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {industries.map((ind) => (
          <div 
            key={ind.id}
            className={`bg-white border-2 rounded-xl p-6 relative transition-all cursor-pointer hover:shadow-md ${ind.active ? 'border-blue-600 shadow-sm' : 'border-gray-200'}`}
          >
            {ind.active && (
            ...
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-5 ${ind.active ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-400'}`}>
              {ind.icon}
            </div>
            ...
            <div className="absolute bottom-4 right-4 text-gray-100 opacity-50">
              <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm-2 10h-4v4h-4v-4H5V9h4V5h4v4h4v4z" />
              </svg>
            </div>
          </div>
        ))}
